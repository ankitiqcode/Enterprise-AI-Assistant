// src/components/documents/DocumentPagination.jsx
import PropTypes from 'prop-types';
import { FiChevronLeft, FiChevronRight } from 'react-icons/fi';

/**
 * Reusable pagination footer for the Documents module.
 *
 * Identical behavior to LeavePagination.jsx (Feature 9) — same
 * sliding 5-button window, same zero-records handling ("Showing 0 of
 * 0 documents" as a distinct display shape, not "0–0 of 0"), same
 * startItem/endItem formulas, same totalPages <= 1 safety — since
 * this spec is functionally the same pagination contract with only
 * the summary copy changed ("documents" instead of "leave
 * requests"). Reused rather than reinvented, as an independent file
 * per your instruction.
 *
 * No API calls, no filtering, no mock data, no navigation — purely
 * presentational. Does not modify DocumentsList.jsx,
 * DocumentTable.jsx, or DocumentFilters.jsx.
 */
...
const DocumentPagination = ({ currentPage, totalPages, totalItems, pageSize, onPageChange }) => {
  ...
  return (
    ...
    >
      {/* Left section — showing summary. Zero-records case renders a
      ...
      {hasRecords ? (
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          ...
          <span className="font-medium text-gray-700 dark:text-gray-300">0</span> documents
        </p>
      )}
