// src/components/departments/DepartmentPagination.jsx
import PropTypes from 'prop-types';
import { FiChevronLeft, FiChevronRight } from 'react-icons/fi';

/**
 * Reusable pagination footer for the Departments module.
 *
 * Unlike EmployeePagination.jsx (Feature 6), which renders a single
 * current-page indicator per its spec, this component renders actual
 * numbered page buttons (max 5 visible at once) per your explicit
 * "Page numbers" + "Show maximum 5 page buttons" requirements here —
 * a deliberate difference between the two modules' pagination UI,
 * not an inconsistency.
 *
 * No API calls, no backend logic — purely presentational, driven by
 * the five props below. DepartmentsList.jsx, DepartmentTable.jsx,
 * and DepartmentFilters.jsx are NOT modified, per your instruction.
 */

/**
 * Computes a sliding window of up to `maxVisible` page numbers,
 * centered on the current page where possible, clamped to
 * [1, totalPages] at the edges so the window never runs short at the
...
const DepartmentPagination = ({ currentPage, totalPages, totalItems, pageSize, onPageChange }) => {
  const isFirstPage = currentPage <= 1;
  const isLastPage = currentPage >= totalPages;

  // Range shown, e.g. "Showing 11–20 of 47 departments". Clamped so
  // a partial final page reports its true end index.
